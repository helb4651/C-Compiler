...
char const *Types[] = {"type void", "type int", "type bool", "type char", "type string", "type record", "undefined type"};
                        ...
                        case OpK:
                        {
                                ...
                                if(strcmp(t->attr.name, "[")==0) {
                                        ...
                                        if(strcmp(t->attr.name, "[")==0 && t->child[0]!=NULL) {
                                                TreeNode *nodeIdK = static_cast<TreeNode *>(semanticsSymbolTable.lookup((char *) t->child[0]->attr.name));
                                                if (nodeIdK != NULL) {
...
//                                        printf("   child[0]: %s\n", t->child[0]->attr.name);
                                ...
                                else if(strcmp(t->attr.name, "and")==0 || strcmp(t->attr.name, "or")==0 || strcmp(t->attr.name, "+")==0 || strcmp(t->attr.name, "-")==0 ||
                                        strcmp(t->attr.name, "*")==0 || strcmp(t->attr.name, "/")==0 || strcmp(t->attr.name, "%")==0 || strcmp(t->attr.name, "not")==0  ||
                                        strcmp(t->attr.name, ">=")==0 || strcmp(t->attr.name, "<=")==0 || strcmp(t->attr.name, "==")==0 || strcmp(t->attr.name, "!=")==0 ||
                                        strcmp(t->attr.name, ">")==0 || strcmp(t->attr.name, "<")==0 || strcmp(t->attr.name, "?")==0 )
                                {
...
//                                printf("Call: %s ", t->attr.name);
                        ...
                        case FuncK:
                        {
                                ...
                                // else { t->declType=UndefinedType; }

                                break;
                        }
                        case VarK:
                                // if(t->child[0]!=NULL) {
                                //         if(t->child[0]->nodekind==ExprK && t->child[0]->kind.expr==IdK) {
                                //                 TreeNode* node = static_cast<TreeNode*>(semanticsSymbolTable.lookup((char *)t->child[0]->attr.name));
                                //                 if(strcmp(node->attr.name, t->child[0]->attr.name)==0) {
                                //                         printf("ERROR(%d): Symbol '%s' is not defined.\n", t->linenum, t->attr.name);
                                //                         t->child[0]->declType=UndefinedType;
                                //                 }
                                //         }
